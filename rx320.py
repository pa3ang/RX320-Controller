...
class RX320():
    ...
    def __init__(self, port, sleep_time=0.2, freq_offset=0, cw_pitch=0): 
        self.ser = serial.Serial(port, 1200, timeout=1)
        self.strength = 0
        self.firmware = ''
        self.freq_offset = freq_offset
        self.cw_pitch = cw_pitch
        thr = threading.Thread(target=self.read_thread, daemon=True)
        thr.start()
        
        thr = threading.Thread(target=self.strength_thread, args=(sleep_time,), daemon = True)
        thr.start()
    ...
    def set_freq(self, freq, cwbfo=None):
        ...
        adjusted_freq = freq - 1250 + mcor*fcor
